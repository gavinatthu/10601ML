...
class LR():
    ...
    def SGD(self, label, feat, w, lr):
        ...
        N = len(self.labels)
        pred = sigmoid(torch.dot(feat, w.T))
        #label = label.astype(torch.float64)                            # float精度转换，否则下一步会出错
    ...
    def train(self):
        ...
        self.feats = torch.from_numpy(self.feats).to(device)
        ...
        self.validfeats = torch.from_numpy(self.validfeats).to(device)
        self.validlabels = torch.from_numpy(self.validlabels).to(device)


        w = torch.zeros(self.feats.shape[1],dtype=float).to(device)                  # 0初始化权重矩阵(M+1)维 (301, )
        self.lls = []
        self.vlls = []
        for epoch in tqdm(range(self.epochs)):
            ...
            for i in range(len(self.labels)):
                ...
                ll += label*torch.log(sigmoid(torch.dot(feat,w.T)))+\
                    (1-label)*torch.log(sigmoid(1-torch.dot(feat,w.T)))
                w, _ = self.SGD(label, feat, w, self.lr)
            
            for i in range(len(self.validlabels)):
                label, feat = self.validlabels[i], self.validfeats[i]
                vll += label*torch.log(sigmoid(torch.dot(feat,w.T)))+\
                    (1-label)*torch.log(sigmoid(1-torch.dot(feat,w.T)))
        ...
        self.lls = np.array(self.lls.cpu()) 
            #self.test()
        self.weight = w
        return self.lls
