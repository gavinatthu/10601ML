...
class HMM():
    ...
    def forward(self, words):
        ...
        for i in range(1, len(words)):
            word = words[i]
            word_ind = self.w2i[word]
            alpha[i, :] = self.emit[:, word_ind] * np.dot(alpha[i - 1, :], self.trans)
        return alpha
    ...
    def backward(self, words):
        ...
        while i >= 0:
            word = words[i + 1]
            word_ind = self.w2i[word]
            beta[i, :] = np.dot(self.emit[:, word_ind] * beta[i + 1, :], self.trans)
            i -= 1
        return beta
...
if __name__ == '__main__':
    '''
    mac用:
    python3 forwardbackwardWebber(1).py ./en_data/train.txt ./en_data/index_to_word.txt \
